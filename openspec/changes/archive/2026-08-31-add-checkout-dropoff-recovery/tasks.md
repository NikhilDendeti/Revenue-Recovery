...
- [x] 1.1 Add `Transaction.Kind.CHECKOUT_DROPOFF`, `checkout_initiated_at`
      ...
      to other fields).
- [x] 1.2 Generate and review the resulting migration file (next number after
...
- [x] 1.3 Add `CHECKOUT_DROPOFF_AT_RISK_HOURS` (default `1.0`) and `HIGH_VALUE_CART_INR`
...
- [x] 2.1 Add `"checkout.abandoned": Transaction.Kind.CHECKOUT_DROPOFF` to
...
- [x] 3.1 Implement `_heuristic_checkout_dropoff_diagnosis(txn)` in
...
- [x] 3.2 Add the `checkout_dropoff` branch to `_heuristic_decision` (always
...
- [x] 3.3 Extend the diagnosis and decision LLM system prompts in
...
- [x] 3.4 Extend the plain-dict `transaction_fields` built in `_run_recovery_pipeline`
...
- [x] 4.1 Add `checkout_dropoff` guardrail-coverage tests to
...
- [x] 5.1 Add `_seed_checkout_dropoff(n)` to
...
- [x] 5.2 Add `backend/recovery/tests/test_seed_data.py` (new file — none exists yet for
...
- [x] 6.1 Add a `checkout_dropoff` entry to `KIND_META` and `KIND_FILTERS` in
...
- [x] 6.2 Add a new `cart` glyph to the `STROKE` map in
...
- [x] 7.1 Run the full backend suite (`pytest`, from `backend/`) and confirm every
      existing test still passes alongside all new tests added above.
- [x] 7.2 Seed and replay locally (`python manage.py seed_data --checkout-dropoff 14`
