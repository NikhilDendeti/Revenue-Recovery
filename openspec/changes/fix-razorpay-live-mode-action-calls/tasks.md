...
- [ ] 1.1 Add `customer_email = models.EmailField(blank=True)` to `Transaction`
      ...
      trigger.
- [ ] 1.2 Update `seed_data.py`'s `_customer()` helper to populate
      ...
      is non-blank.
- [ ] 1.3 Update `WebhookView.post` (`backend/recovery/views.py`, alongside the
...
- [ ] 2.1 Widen `reopen_order_checkout`'s signature to `reopen_order_checkout(
...
- [ ] 2.2 Rewrite its live-mode body (the `_configured()` True branch) to call
...
- [ ] 2.3 Change its simulated-mode branch to return a payment-link-shaped response
...
- [ ] 2.4 Update `tasks.py::_call_razorpay`'s `RETRY_ORDER` branch to pass
...
- [ ] 2.5 Update `test_razorpay_client.py::test_reopen_order_checkout_is_simulated`
...
- [ ] 2.6 Add a `TestLiveMode` case exercising `reopen_order_checkout` for real
...
- [ ] 3.1 Add a `customer_email: str` parameter to `create_registration_link`
...
- [ ] 3.2 In the live-mode body: raise `RazorpayError("create_registration_link
      ...
      `TestSimulatedMode`).
- [ ] 3.3 Otherwise, build the payload with `"customer": {"name": customer_name,
...
- [ ] 3.4 Add a `TestLiveMode` case calling `create_registration_link` for real (test
...
- [ ] 3.5 Update `test_razorpay_client.py::test_create_registration_link_is_simulated`
...
- [ ] 4.1 Update `recovery/interfaces/ports.py`'s `PaymentGatewayInterface.
...
- [ ] 4.2 Update `recovery/adapters/razorpay_gateway.py`'s `RazorpayGateway.
      ...
      "+919821123456")`.
- [ ] 4.3 Update `recovery/tests/fakes.py`'s `FakeGateway.reopen_order_checkout` to
...
- [ ] 5.1 Run the full backend suite (`cd backend && .venv/Scripts/python.exe -m
...
- [ ] 5.2 With no live keys configured, run `python manage.py seed_data --flush`
